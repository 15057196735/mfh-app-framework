...
public class DefaultJSONParser extends AbstractJSONParser implements Closeable {
    ...
    public final static ThreadLocal<DateFormat> dateFormatPatternThread = new ThreadLocal<DateFormat> ();//线程变量，外部传入，打个补丁。add by zhangyz
    ...
    public DateFormat getDateFormat() {
        DateFormat format = dateFormatPatternThread.get();
        if (format != null)
            return format;//add by zhangyz
