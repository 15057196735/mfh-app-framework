...
public class SqlBuilder {
	...
	public static List<KeyValue> getSaveKeyValueListByEntity(Object entity){
		...
		if (idvalue != null) {//建表时已经注释掉了自增长特性，update by zhangyz 20130607
		    KeyValue kv = new KeyValue(table.getId().getColumn(),idvalue);
            keyValueList.add(kv);
		}
    ...
    public static SqlInfo getSelectCountAsSqlInfo(Class<?> clazz,Object idValue){
        TableInfo table=TableInfo.get(clazz);
	...
	public static String getSelectCountSQL(Class<?> clazz){
        return getSelectCountSqlByTableName(TableInfo.get(clazz).getTableName());
    }
	...
	public static String getCreatTableSQL(Class<?> clazz){
        ...
        String keyColType = "";
		if( primaryClazz == int.class || primaryClazz == Integer.class || primaryClazz == Long.class)
            keyColType = "INTEGER";// AUTOINCREMENT
		/*else if (primaryClazz instanceof Object) {
            if (IStringId.class.isAssignableFrom(clazz))
                keyColType = "TEXT";
            else if (ILongId.class.isAssignableFrom(clazz) || IIntId.class.isAssignableFrom(clazz))
                keyColType = "INTEGER";
            else
                throw new RuntimeException(clazz.getName() + "应从IString、ILong或IInt等接口继承");
        }*/
        else if (primaryClazz == String.class)
            keyColType = "VARCHAR";
        else
            keyColType = "TEXT";
		strSQL.append("\"").append(id.getColumn()).append("\" ").append(keyColType).append(" PRIMARY KEY NOT NULL,");
		
		Collection<Property> propertys = table.propertyMap.values();
		for(Property property : propertys){
		    if (property.getSet() != null) {
    			strSQL.append("\"").append(property.getColumn()).append("\"");
    			strSQL.append(" ").append(getNativeTypeFormJb(property.getDataType()));
    			strSQL.append(",");
		    }
		}
	...
	private static String getNativeTypeFormJb(Class<?> primaryClazz) {
        if (primaryClazz == int.class || primaryClazz == Integer.class || primaryClazz == Long.class || primaryClazz == long.class)          
            return "INTEGER";
        else if (primaryClazz == String.class)
            return "VARCHAR";
        else if (primaryClazz == Double.class || primaryClazz == double.class)
            return "DOUBLE";
        else if (primaryClazz == Float.class || primaryClazz == float.class)
            return "FLOAT";
        else if (primaryClazz == BigInteger.class)
            return "INTEGER";
        else if (primaryClazz == BigDecimal.class)
            return "NUMERIC";
        else if (primaryClazz == java.util.Date.class || primaryClazz == java.sql.Date.class)
            return "DATETIME";
        else if (primaryClazz == Time.class)
            return "TIME";
        else if (primaryClazz == Timestamp.class)
            return "TIMESTAMP";
        else if (primaryClazz == Boolean.class || primaryClazz == boolean.class)
            return "BOOLEAN";
        /*else if (primaryClazz == java.io.Reader)
            return "TEXT";*/
        else if (primaryClazz == byte[].class)
            return "BLOB";
        else 
            return "VARCHAR";
    }
